...
pitchers_debut %>%
  ...
  filter(ERA < 100) %>% #removes single pitcher with an ERA of Inf
  summarize(mean_IP = round(mean(IP)), 
            median_whip = round(median(WHIP, na.rm=TRUE), 2), 
            mean_WAR = round(mean(WAR, na.rm=TRUE), 1), 
            mean_ASG = round(mean(ASG, na.rm=TRUE), 2), 
            median_ERA = round(median(ERA, na.rm=TRUE), 1), #due mean weighted by innings pitched
            mean_G = round(mean(G, na.rm=TRUE), 1))
```
...
Next steps: get season by season data in, build aging curve, compare stats before and after Tommy John surgery. 
