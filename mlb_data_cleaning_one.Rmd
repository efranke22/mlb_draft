...
# read in pitcher debuts and remove totals from the pitchers column and strip "HOF" from the hall of framers names
pitch_debuts <- read_rds("data/pitch_debuts.rds") %>%
  filter(Name != "Totals") %>%
  mutate(Name = str_remove_all(Name, " HOF"))

# read in all position player debuts since 1987
all_debuts <- read_rds("data/debuts.rds")

#read in tommy john surgeries 
tommyjohn <- read_xlsx("data/tommyjohndata.xlsx") %>%
  filter(Position == "P") %>%
  select(Player, `TJ Surgery Date`, Level, `Return Date (same level)`, `Recovery Time (months)`, `Surgeon(s)`, `Post-TJ MLB G`, `Post-TJ MLB IP/PA`, `Active`) %>%
  rename(surgery_date = `TJ Surgery Date`, 
         tj_level = Level, 
         tj_return_same_level = `Return Date (same level)`, 
         tj_recovery_months = `Recovery Time (months)`, 
         tj_surgeon = `Surgeon(s)`, 
         post_tj_mlb_g = `Post-TJ MLB G`, 
         post_tj_mlb_ip_per_pa = `Post-TJ MLB IP/PA`, 
         active = Active)
```
...
Join with tommy john data. Create an indicator variable that indicates if the player has had tommy john surgery. Create indicator variable for if the player had surgery. Create variable for approximate draft date, as well as a variable indicating when the surgery took place. 
```{r}
